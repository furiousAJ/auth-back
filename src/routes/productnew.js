...
app.post('/productnew', authenticateSellerToken, upload.single('Photo'), async (req, res) => {
    ...
    app.get('/sellerproductview', authenticateSellerToken, async (request, response) => {
        try {
            const sellerId = request.seller._id;
            const products = await ProductModel.find({ seller: sellerId }).populate('prod', 'Categoryname');
